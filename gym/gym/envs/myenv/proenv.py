...
class procon18Env(gym.Env): #define environment
    ...
    def __init__(self): #initialization
        ...
        self._1reward_range = [-5.,10.] #報酬の最小値と最大値のリスト
        ...
        self._2reward_range = [-5.,10.] #報酬の最小値と最大値のリスト
        self._reset(1)
    ...
    def _reset(self,one): # initialization of position,points and steps  (rv is array of position)
        if one == 1:
            pass
        else:
            fs = self.makeField()
        ...
        self.points = 0
        self.steps = 0
        return self._observe('F')

    def _step(self, action, QoM=None): # processing of 1step (rv is observation,reward,done,info)
        if QoM is None:
            if action[0][1] == "move":
                self.Move(3, action[0][0])
            elif action[0][1] == "remove":
                self.Remove(3, action[0][0])

            if action[1][1] == "move":
                self.Move(4, action[1][0])
            elif action[1][1] == "remove":
                self.Remove(4, action[1][0])
            observation = self._observe('E')
            return observation

        else:
            if action[0][1] == "move":
                self.Move(1, action[0][0])
            elif action[0][1] == "remove":
                self.Remove(1, action[0][0])
            if action[1][1] == "move":
                self.Move(2, action[1][0])
            elif action[1][1] == "remove":
                self.Remove(2, action[1][0])
                
            observation = self._observe('F') #array
            if QoM is "Q":
                reward = self._get_reward_QL() #str
            elif QoM is "M":
                reward = self._get_reward_MCM()
            self.done = self._is_done() #num
            return observation, float(int(reward)), self.done, {}
    ...
    def _get_reward_MCM(self):
        if self._is_done():
            if self.judVoL() == "Win_1":
                return "10"
            else:
                return "-5"
        else:
    ...
    def _observe(self, type): #position of agent (array)  // verified 味方依存
        if type == 'F':
            observation = np.array([self.getPosition(1), self.getPosition(2)])
        elif type == 'E':
            observation = np.array([self.getPosition(3), self.getPosition(4)])
        return observation  # [[x(column),y(row)],[x,y]]
    ...
    def calcPoint(self): # Calculate Point  array  // verified
        ...
        p = np.array([self.friend_tilePoint, self.friend_territoryPoint, self.friend_totalPoint, self.enemy_tilePoint, self.enemy_territoryPoint, self.enemy_totalPoint])
        return p
    ...
    def getPosition(self, usr): #get position (array)  // verified
        ...
        f = response.text.encode('utf-8').decode().replace("\n", " ").replace("  "," ")
        pos_array = np.array([int(i) for i in f.split()])
