// learning only port num is 8004
package main
...
func MoveServer(w http.ResponseWriter, r *http.Request) {
    // fmt.Fprintf(w, "move\n") yusaku
    r.ParseForm()
    //curl -X POST localhost:8004/move -d "usr=1&d=right"
...
func JudgeServer(w http.ResponseWriter, r *http.Request) { // ;;;
    // fmt.Fprintf(w, "move\n") yusak
    // curl -X POST localhost:8004/judgedirection -d "usr=1&d=r"
    r.ParseForm()
    //curl -X POST localhost:8004/move -d "usr=1&d=right"
    u,_:=strconv.Atoi(r.FormValue("usr"))
    fmt.Println(u)
...
func main() {
    ...
    // ログ出力
    log.Printf("Start Go HTTP Server (port number is 8004,learning only)")
